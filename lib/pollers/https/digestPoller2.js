...
var webviewAsserts = require('../../webviewAsserts.js');
...
DigestPoller.type = 'digest-bbia-vpa-10.3.0i';
...
DigestPoller.prototype.handleOkResponse = function(res) {
    ...
    console.log('bodyObj');
    console.log(bodyObj);
    for(var i = 0; i < bodyObj['entities'].length; i++) {
        var entity = bodyObj['entities'][i];
        if(entity.type == 'WEBVIEWURL') {
            webviewUri = entity.value;
        }
    }
    if(webviewUri == null) {
        poller.callback(undefined, poller.getTime(), res);
    } else {
	console.log('webviewUri found');
	var that = this;
        webviewAsserts.checkValidWebview(webviewUri, function (err) { //error
	  that.onErrorCallback({ name: 'WebviewFailed', message: 'Webview failed'});
	}, function () { //success
	  poller.callback(undefined, poller.getTime(), res); //do the same success callback that would have been done otherwise; we only reach here if selenium was successful anyway
	});
    }
};
