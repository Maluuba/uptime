...
var webviewAsserts = require('../lib/webviewAsserts');
...
DigestPoller.prototype.handleOkResponse = function(res) {
    ...
    var webviewUri = null;
    for(var i = 0; i < res.body['entities'].length; i++) {
        var entity = res.body['entities'][i];
        if(entity.type == 'WEBVIEW') {
    ...
    if(webviewUri == null) {
        ...
        webviewAsserts.checkValidWebview(webviewUri, function (err) {
            console.log(err);
        }, poller.callback);
    }
};
